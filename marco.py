...
# Cell 12: Fixed Detailed Anomaly Detection with Realistic Anomaly Counts
...
# Force matplotlib settings
import matplotlib
matplotlib.use('Agg')
plt.ioff()

print("🚀 CREATING REALISTIC ANOMALY DETECTION WITH MORE ANOMALIES!")

# STEP 1: CREATE DIRECTORY
base_path = "common_results/marco_results/visualizations"
os.makedirs(base_path, exist_ok=True)
print(f"✅ Directory: {os.path.abspath(base_path)}")

# STEP 2: CREATE MORE REALISTIC AUTOMOTIVE DATA WITH MORE ANOMALIES
print("📊 Creating realistic automotive data with increased anomaly density...")
np.random.seed(42)
n = 3000

# Create realistic automotive time series data with MORE anomalies
time_indices = np.arange(n)

# Voltage data (realistic automotive voltage patterns) - MORE ANOMALIES
voltage_base = 12.6 + 0.4 * np.sin(time_indices * 0.01) + np.random.normal(0, 0.15, n)
# Increase anomaly count significantly
n_voltage_anomalies = 150  # 5% anomalies
voltage_anomalies = np.random.choice(n, n_voltage_anomalies, replace=False)
voltage_data = voltage_base.copy()
# More varied anomaly types
for i, idx in enumerate(voltage_anomalies):
    if i % 4 == 0:  # Severe spikes
        voltage_data[idx] = voltage_base[idx] + np.random.choice([5, 6, 7, 8])
    elif i % 4 == 1:  # Severe drops
        voltage_data[idx] = voltage_base[idx] + np.random.choice([-4, -5, -6])
    elif i % 4 == 2:  # Medium spikes
        voltage_data[idx] = voltage_base[idx] + np.random.choice([2.5, 3, 3.5])
    else:  # Medium drops
        voltage_data[idx] = voltage_base[idx] + np.random.choice([-2.5, -3, -3.5])

# Current data (realistic current patterns) - MORE ANOMALIES
current_base = 2.5 + 1.0 * np.sin(time_indices * 0.005) + np.random.normal(0, 0.25, n)
n_current_anomalies = 120  # 4% anomalies
current_anomalies = np.random.choice(n, n_current_anomalies, replace=False)
current_data = current_base.copy()
for i, idx in enumerate(current_anomalies):
    if i % 3 == 0:  # Overcurrent
        current_data[idx] = current_base[idx] + np.random.choice([6, 8, 10, 12])
    elif i % 3 == 1:  # Undercurrent
        current_data[idx] = current_base[idx] + np.random.choice([-4, -6, -8])
    else:  # Medium anomalies
        current_data[idx] = current_base[idx] + np.random.choice([3, 4, -3, -4])

# Temperature data (realistic temperature patterns) - MORE ANOMALIES
temp_base = 85 + 10 * np.sin(time_indices * 0.003) + np.random.normal(0, 3, n)
n_temp_anomalies = 100  # 3.3% anomalies
temp_anomalies = np.random.choice(n, n_temp_anomalies, replace=False)
temp_data = temp_base.copy()
for i, idx in enumerate(temp_anomalies):
    if i % 3 == 0:  # Overheating
        temp_data[idx] = temp_base[idx] + np.random.choice([25, 30, 35, 40])
    elif i % 3 == 1:  # Undercooling
        temp_data[idx] = temp_base[idx] + np.random.choice([-20, -25, -30])
    else:  # Medium temp anomalies
        temp_data[idx] = temp_base[idx] + np.random.choice([15, 20, -15, -18])

# SOC data (State of Charge) - MORE ANOMALIES
soc_base = 75 + 15 * np.sin(time_indices * 0.002) + np.random.normal(0, 2, n)
soc_base = np.clip(soc_base, 0, 100)
n_soc_anomalies = 80  # 2.7% anomalies
soc_anomalies = np.random.choice(n, n_soc_anomalies, replace=False)
soc_data = soc_base.copy()
for i, idx in enumerate(soc_anomalies):
    if i % 4 == 0:  # Critical low
        soc_data[idx] = np.random.choice([2, 5, 8, 10])
    elif i % 4 == 1:  # Overcharged
        soc_data[idx] = np.random.choice([95, 98, 99, 100])
    elif i % 4 == 2:  # Low battery
        soc_data[idx] = np.random.choice([15, 18, 20, 22])
    else:  # High charge anomalies
        soc_data[idx] = np.random.choice([88, 90, 92, 94])
...
print(f"✅ Created automotive dataset with {n} samples")
print(f"📊 Expected anomalies: ~{n_voltage_anomalies + n_current_anomalies + n_temp_anomalies + n_soc_anomalies}")

# STEP 3: ENHANCED ANOMALY DETECTION WITH LOWER THRESHOLDS
def detect_detailed_anomalies(data, feature_name):
    """Detect anomalies with MORE SENSITIVE detection and detailed explanations"""
    ...
    # Multiple detection methods with LOWER THRESHOLDS for more anomalies
    # Method 1: Statistical (LOWERED threshold from 2.5 to 2.0)
    statistical_threshold = 2.0  # More sensitive
    statistical_anomalies = np.where(z_scores > statistical_threshold)[0]
    
    # Method 2: Isolation Forest with HIGHER contamination
    iso_forest = IsolationForest(contamination=0.05, random_state=42)  # 5% instead of 1%
    iso_predictions = iso_forest.fit_predict(data.reshape(-1, 1))
    iso_anomalies = np.where(iso_predictions == -1)[0]
    
    # Method 3: IQR-based detection (additional method)
    ...
    for idx in all_anomalies:
        z_score = float(z_scores[idx])  # Convert to native Python float
        original_value = float(data[idx])  # Convert to native Python float
        scaled_value = float(data_scaled[idx])  # Convert to native Python float
        
        # Determine detection method
        method = []
        if idx in statistical_anomalies:
            method.append("statistical")
        ...
        # Feature-specific explanations with realistic thresholds
        if 'Voltage' in feature_name:
            if original_value > 16:
                explanation = f"Critical overvoltage detected (ratio: {z_score:.2f})"
                reason = "CRITICAL electrical fault"
            elif original_value < 10:
                explanation = f"Critical undervoltage detected (ratio: {z_score:.2f})"
                reason = "CRITICAL electrical fault"
            elif original_value > 14:
                explanation = f"High voltage spike detected (ratio: {z_score:.2f})"
                reason = "HIGH voltage anomaly"
            elif original_value < 11:
                explanation = f"Low voltage drop detected (ratio: {z_score:.2f})"
                reason = "HIGH voltage anomaly"
            else:
                explanation = f"Voltage fluctuation detected (ratio: {z_score:.2f})"
                reason = "MEDIUM voltage anomaly"
        elif 'Current' in feature_name:
            if abs(original_value) > 10:
                explanation = f"Critical overcurrent detected (ratio: {z_score:.2f})"
                reason = "CRITICAL current fault"
            elif abs(original_value) > 6:
                explanation = f"High current anomaly detected (ratio: {z_score:.2f})"
                reason = "HIGH current anomaly"
            else:
                explanation = f"Current fluctuation detected (ratio: {z_score:.2f})"
                reason = "MEDIUM current anomaly"
        elif 'Temperature' in feature_name:
            if original_value > 120:
                explanation = f"Critical overheating detected (ratio: {z_score:.2f})"
                reason = "CRITICAL thermal fault"
            elif original_value < 50:
                explanation = f"Critical undertemperature detected (ratio: {z_score:.2f})"
                reason = "CRITICAL thermal fault"
            elif original_value > 110:
                explanation = f"High temperature detected (ratio: {z_score:.2f})"
                reason = "HIGH thermal anomaly"
            elif original_value < 60:
                explanation = f"Low temperature detected (ratio: {z_score:.2f})"
                reason = "HIGH thermal anomaly"
            else:
                explanation = f"Temperature fluctuation detected (ratio: {z_score:.2f})"
                reason = "MEDIUM thermal anomaly"
        elif 'SOC' in feature_name:
            if original_value < 10:
                explanation = f"Critical battery depletion (ratio: {z_score:.2f})"
                reason = "CRITICAL battery fault"
            elif original_value > 97:
                explanation = f"Critical battery overcharge (ratio: {z_score:.2f})"
                reason = "CRITICAL battery fault"
            elif original_value < 20:
                explanation = f"Low battery warning (ratio: {z_score:.2f})"
                reason = "HIGH battery anomaly"
            elif original_value > 90:
                explanation = f"High charge level detected (ratio: {z_score:.2f})"
                reason = "HIGH battery anomaly"
            else:
                explanation = f"SOC fluctuation detected (ratio: {z_score:.2f})"
                reason = "MEDIUM battery anomaly"
        else:
            explanation = f"Statistical anomaly {direction} threshold (ratio: {z_score:.2f})"
            reason = f"{severity} outlier"
        ...
        anomaly_details.append({
            ...
            'reason': reason,
            'threshold': float(statistical_threshold)  # Convert to native Python float
        })
    
    return anomaly_details, data_scaled, z_scores

# STEP 4: CREATE DETAILED VISUALIZATION LIKE YOUR IMAGES
def create_detailed_anomaly_chart(data, feature_name, anomaly_details, data_scaled, save_path):
    """Create detailed anomaly chart like your reference images"""
    
    # Sort anomalies by severity for better visualization
    # Include more severity levels for more plots
    high_priority_anomalies = [a for a in anomaly_details if a['severity'] in ['CRITICAL', 'HIGH']]
    
    # Create subplots - show top 10 most critical anomalies
    n_plots = min(10, len(high_priority_anomalies))
    
    if n_plots == 0:
        # If no high priority, show medium ones
        high_priority_anomalies = [a for a in anomaly_details if a['severity'] in ['MEDIUM', 'LOW']]
        n_plots = min(10, len(high_priority_anomalies))
    ...
    fig.suptitle(f'Top {n_plots} Anomaly Explanations with Context - {feature_name}', 
                 fontsize=16, fontweight='bold')
    ...
    for i in range(n_plots):
        ax = axes[i]
        anomaly = high_priority_anomalies[i]
        idx = anomaly['index']
        ...
        ax.plot(window_indices, window_data, 'b-', linewidth=1, alpha=0.7, 
               label='A Current' if 'Current' in feature_name else f'A {feature_name.split("_")[1]}')
        ...
        color_map = {'CRITICAL': 'red', 'HIGH': 'orange', 'MEDIUM': 'yellow', 'LOW': 'lightgreen'}
        point_color = color_map.get(anomaly['severity'], 'red')
        
        # Highlight the anomaly point
        ax.plot(idx, anomaly['scaled_value'], 'o', markersize=8, markerfacecolor=point_color, 
        ...
        ax.set_title(f'{feature_name}\nZ={anomaly["z_score"]:.2f}, {anomaly["severity"]}\n'
                    f'Value: {anomaly["value"]:.2f}\nReason: {anomaly["reason"]}', 
                    fontsize=9, fontweight='bold')
        
        # Add method and explanation text box
        textstr = f'Method: {anomaly["method"]}\n'
        textstr += f'Z-Score: {anomaly["z_score"]:.2f}\n'
        textstr += f'Detection: {anomaly["method"]}\n'
        textstr += f'Val: {anomaly["value"]:.2f}'
        ...
        # Set axis labels
        ax.set_xlabel(f'Sample Index (around {idx})', fontsize=8)
    ...
    if os.path.exists(save_path):
        ...
        print(f"   ✅ SAVED: {os.path.basename(save_path)} ({size:,} bytes)")
        return True
    return False

# STEP 5: PROCESS EACH FEATURE WITH ENHANCED DETECTION
print("\n🔬 Processing each automotive feature with enhanced detection...")
...
for feature_name in automotive_data.columns:
    ...
    # Detect anomalies with enhanced sensitivity
    anomaly_details, data_scaled, z_scores = detect_detailed_anomalies(feature_data, feature_name)
    ...
    all_anomaly_results[feature_name] = {
        ...
        'critical_count': len([a for a in anomaly_details if a['severity'] == 'CRITICAL']),
        'high_count': len([a for a in anomaly_details if a['severity'] == 'HIGH']),
        'medium_count': len([a for a in anomaly_details if a['severity'] == 'MEDIUM']),
        'low_count': len([a for a in anomaly_details if a['severity'] == 'LOW'])
    }
    
    # Create detailed chart
    chart_path = os.path.join(base_path, f'{feature_name}_detailed_anomalies.png')
    create_detailed_anomaly_chart(feature_data, feature_name, anomaly_details, data_scaled, chart_path)

# STEP 6: CREATE SEVERITY BREAKDOWN CHARTS
def create_severity_breakdown_chart(all_results, save_path):
    """Create severity breakdown chart like the second image"""
    
    # Count LOW and MEDIUM severity anomalies for detailed view
    low_medium_anomalies = []
    for feature_name, results in all_results.items():
        for anomaly in results['anomalies']:
            if anomaly['severity'] in ['LOW', 'MEDIUM']:
                low_medium_anomalies.append({
    ...
    low_medium_anomalies.sort(key=lambda x: x['z_score'], reverse=True)
    low_medium_anomalies = low_medium_anomalies[:10]
    
    if len(low_medium_anomalies) == 0:
        print("   No low/medium severity anomalies found")
        return
    ...
    rows = 2
    fig, axes = plt.subplots(rows, cols, figsize=(20, 8))
    total_anomalies = sum(results['anomaly_count'] for results in all_results.values())
    fig.suptitle(f'LOW/MEDIUM Severity Anomalies - Detailed Explanations ({len(low_medium_anomalies)} of {total_anomalies})', 
                 fontsize=16, fontweight='bold', color='orange')
    
    axes = axes.flatten()
    
    for i, anomaly in enumerate(low_medium_anomalies):
        ax = axes[i]
        ...
        scaler = StandardScaler()
        data_scaled = scaler.fit_transform(feature_data.reshape(-1, 1)).flatten()
        ...
        window_size = 50
        start_idx = max(0, idx - window_size)
        end_idx = min(len(feature_data), idx + window_size)
        window_indices = np.arange(start_idx, end_idx)
        window_data = data_scaled[start_idx:end_idx]
        ...
        # Add threshold lines
        ax.axhline(y=2.0, color='orange', linestyle='--', alpha=0.7, linewidth=1)
        ax.axhline(y=-2.0, color='orange', linestyle='--', alpha=0.7, linewidth=1)
        
        # Color code by severity
        point_color = 'orange' if anomaly['severity'] == 'MEDIUM' else 'yellow'
        marker = 'o' if anomaly['severity'] == 'MEDIUM' else 's'
        
        # Highlight anomaly point
        ax.plot(idx, data_scaled[idx], marker, color=point_color, markersize=8, 
               markeredgecolor='black', markeredgewidth=1)
        
        # Vertical line
        ax.axvline(x=idx, color='green', linestyle='-', alpha=0.6, linewidth=1)
        
        # Title
        ax.set_title(f'{anomaly["severity"]} - Combined\nZ-Score: {anomaly["z_score"]:.2f} | Method: {anomaly["method"]}\n'
                    f'Value: {anomaly["value"]:.2f}, Feature: {feature_name}', 
                    fontsize=8, fontweight='bold', color=point_color)
        
        # Info box
        textstr = f'Value: {anomaly["value"]:.5f}\n'
        textstr += f'Threshold: 2.0\n'
        textstr += f'Detection: {anomaly["method"]}\n'
        textstr += f'Severity: {anomaly["severity"]}'
        ...
        # Labels
        ax.set_xlabel(f'Sample Index (Window around {idx})', fontsize=7)
    ...
    # Hide unused subplots
    for i in range(len(low_medium_anomalies), len(axes)):
        axes[i].set_visible(False)
...
severity_chart_path = os.path.join(base_path, 'low_medium_severity_anomalies_detailed.png')
create_severity_breakdown_chart(all_anomaly_results, severity_chart_path)

# STEP 7: CREATE SUMMARY REPORT WITH FIXED JSON SERIALIZATION
print("\n📋 Creating detailed summary with proper JSON serialization...")
...
summary = {
    ...
    'total_samples': len(automotive_data),
    'enhanced_detection_settings': {
        'statistical_threshold': 2.0,
        'isolation_forest_contamination': 0.05,
        'iqr_multiplier': 1.2,
        'methods_used': ['statistical', 'isolation_forest', 'iqr']
    },
    'detailed_analysis': {}
}
...
for feature_name, results in all_anomaly_results.items():
    ...
    serializable_anomalies = [convert_to_serializable(anomaly) for anomaly in results['anomalies'][:10]]
    
    summary['detailed_analysis'][feature_name] = {
        'total_anomalies': results['anomaly_count'],
        'critical_anomalies': results['critical_count'],
        'high_anomalies': results['high_count'],
        'medium_anomalies': results['medium_count'],
        'low_anomalies': results['low_count'],
        'anomaly_rate_percent': round((results['anomaly_count'] / results['total_samples'] * 100), 2),
        'top_10_anomalies': serializable_anomalies
    }
...
summary['overall_stats'] = {
    ...
    'severity_distribution': {
        'critical_percent': round((total_critical / total_anomalies * 100), 1) if total_anomalies > 0 else 0,
        'high_percent': round((total_high / total_anomalies * 100), 1) if total_anomalies > 0 else 0,
        'medium_percent': round((total_medium / total_anomalies * 100), 1) if total_anomalies > 0 else 0,
        'low_percent': round((total_low / total_anomalies * 100), 1) if total_anomalies > 0 else 0
    }
}

# Save summary with proper JSON serialization
json_path = os.path.join(base_path, 'detailed_anomaly_analysis.json')
try:
    with open(json_path, 'w') as f:
        json.dump(summary, f, indent=2)
    print(f"✅ JSON SAVED: detailed_anomaly_analysis.json")
except Exception as e:
...
print(f"✅ CREATED {len(files)} DETAILED FILES:")
for file in sorted(files):
    file_path = os.path.join(base_path, file)
    size = os.path.getsize(file_path)
    print(f"   📄 {file} ({size:,} bytes)")

print(f"\n🎉 SUCCESS! Enhanced anomaly detection with realistic results!")
...
# STEP 9: CREATE OVERVIEW SUMMARY CHART
def create_anomaly_overview_chart(all_results, save_path):
    """Create overview chart showing anomaly distribution across features"""
    
    features = list(all_results.keys())
    critical_counts = [all_results[f]['critical_count'] for f in features]
    high_counts = [all_results[f]['high_count'] for f in features]
    medium_counts = [all_results[f]['medium_count'] for f in features]
    low_counts = [all_results[f]['low_count'] for f in features]
    ...
    p1 = ax1.bar(x, critical_counts, width, label='Critical', color='red', alpha=0.8)
    p2 = ax1.bar(x, high_counts, width, bottom=critical_counts, label='High', color='orange', alpha=0.8)
    p3 = ax1.bar(x, medium_counts, width, bottom=np.array(critical_counts) + np.array(high_counts), 
                label='Medium', color='yellow', alpha=0.8)
    p4 = ax1.bar(x, low_counts, width, 
                bottom=np.array(critical_counts) + np.array(high_counts) + np.array(medium_counts),
                label='Low', color='lightgreen', alpha=0.8)
    ...
    for i, feature in enumerate(features):
        total = all_results[feature]['anomaly_count']
        ax1.text(i, total + 1, str(total), ha='center', va='bottom', fontweight='bold')
    
    # Right plot: Pie chart of overall severity distribution
    severity_totals = [sum(critical_counts), sum(high_counts), sum(medium_counts), sum(low_counts)]
    severity_labels = ['Critical', 'High', 'Medium', 'Low']
    colors = ['red', 'orange', 'yellow', 'lightgreen']
    
    # Only include non-zero segments
    non_zero_data = [(label, count, color) for label, count, color in zip(severity_labels, severity_totals, colors) if count > 0]
    ...
    if non_zero_data:
        ...
        for i, (label, count) in enumerate(zip(labels, counts)):
            ...
            if angle > 30:  # Only add count if slice is large enough
...
overview_chart_path = os.path.join(base_path, 'anomaly_overview_summary.png')
create_anomaly_overview_chart(all_anomaly_results, overview_chart_path)

# STEP 10: CREATE DETECTION METHOD COMPARISON CHART
def create_detection_method_comparison(all_results, save_path):
    """Create chart comparing detection methods effectiveness"""
    
    method_stats = {
        'statistical': 0,
        'isolation_forest': 0,
        'iqr': 0,
        'statistical+isolation_forest': 0,
        'statistical+iqr': 0,
        'isolation_forest+iqr': 0,
        'statistical+isolation_forest+iqr': 0
    }
    ...
    for feature_name, results in all_results.items():
        for anomaly in results['anomalies']:
            ...
            if method in method_stats:
                ...
                # Handle any unexpected method combinations
                if 'statistical' in method and 'isolation_forest' in method and 'iqr' in method:
                    method_stats['statistical+isolation_forest+iqr'] += 1
                elif 'statistical' in method and 'isolation_forest' in method:
                    method_stats['statistical+isolation_forest'] += 1
                elif 'statistical' in method and 'iqr' in method:
                    method_stats['statistical+iqr'] += 1
                elif 'isolation_forest' in method and 'iqr' in method:
                    method_stats['isolation_forest+iqr'] += 1
    
    # Filter out zero counts and sort by count
    non_zero_methods = {k: v for k, v in method_stats.items() if v > 0}
    sorted_methods = sorted(non_zero_methods.items(), key=lambda x: x[1], reverse=True)
    ...
    for method in methods:
        if '+' in method:
            colors.append('lightblue')  # Combined methods
    ...
    bars = ax.barh(range(len(methods)), counts, color=colors, alpha=0.8, edgecolor='black')
    
    # Add count labels
    for i, (method, count) in enumerate(zip(methods, counts)):
        ax.text(count + max(counts) * 0.01, i, str(count), va='center', fontweight='bold')
    
    ax.set_yticks(range(len(methods)))
    ax.set_yticklabels([m.replace('+', ' + ').replace('_', ' ').title() for m in methods])
    ax.set_xlabel('Number of Anomalies Detected')
    ax.set_title('Anomaly Detection Method Effectiveness Comparison')
    ax.grid(True, alpha=0.3, axis='x')
    
    # Add legend
    legend_elements = [
        plt.Rectangle((0,0),1,1, facecolor='lightcoral', alpha=0.8, label='Statistical Only'),
        plt.Rectangle((0,0),1,1, facecolor='lightgreen', alpha=0.8, label='Isolation Forest Only'),
        plt.Rectangle((0,0),1,1, facecolor='lightyellow', alpha=0.8, label='IQR Only'),
        plt.Rectangle((0,0),1,1, facecolor='lightblue', alpha=0.8, label='Combined Methods')
    ]
...
method_chart_path = os.path.join(base_path, 'detection_method_comparison.png')
create_detection_method_comparison(all_anomaly_results, method_chart_path)
...
for method, count in sorted(method_stats.items(), key=lambda x: x[1], reverse=True):
    print(f"   📍 {method.replace('+', ' + ')}: {count} anomalies")

print(f"\n✅ ENHANCED ANOMALY DETECTION COMPLETE!")
print(f"📁 All files saved to: {os.path.abspath(base_path)}")
print(f"🎯 REALISTIC DETECTION ACHIEVED: {total_anomalies:,} anomalies from {len(automotive_data):,} samples")
print(f"📈 Average detection rate per feature: {total_anomalies / len(automotive_data.columns):.0f} anomalies")
print(f"🔍 Multiple detection methods successfully implemented")
